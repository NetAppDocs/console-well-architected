...
* You have a link and connector configured for FSx for ONTAP connectivity.
...
| Set the evaluation frequency (minimum 1 hour, default 24 hours) and select *Schedule*. The rule is enabled and runs on the defined schedule.

| Save without scheduling
| Select *Save as draft*. The rule is saved but not evaluated until you schedule it.

| Discard the rule
| Select *Discard* to cancel rule creation and start over.
